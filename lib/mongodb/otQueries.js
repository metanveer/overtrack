...
export async function insertOt(data) {
  ...
  // Ensure _id is not included in the data to be inserted

  const result = await OVERTIME_COLLECTION.insertOne(dataWithoutId);
  return result;
}

export async function checkDuplicateOtEntry(data) {
  const { Date, Type, Unit, Dept, Employee } = data;
...
export async function getYearlyActualOt(deptName, year) {
  const monthlyOt = await OVERTIME_COLLECTION.aggregate([
    // Step 1: Filter by Dept and Year
    {
      $match: {
        Dept: deptName,
        Date: { $regex: `^${year}` },
    ...
    // Step 4: Group by employee and month
    {
      $group: {
        _id: {
          name: "$Employee.Name",
        ...
        totalOt: {
          $sum: {
            $round: [{ $toDouble: "$Employee.OtHour" }, 1],
          },
        },
      },
    },

    // Step 5: Group by employee name
    {
      $group: {
        _id: "$_id.name",
        monthly: {
    ...
    // Step 6: Create monthly object
    {
      $addFields: {
        monthlyObj: { $arrayToObject: "$monthly" },
      },
    },

    // Step 7: Add zero-based months and compute yearly total before NaN conversion
    {
      $addFields: {
        jan: { $ifNull: ["$monthlyObj.1", 0] },
        feb: { $ifNull: ["$monthlyObj.2", 0] },
        mar: { $ifNull: ["$monthlyObj.3", 0] },
        apr: { $ifNull: ["$monthlyObj.4", 0] },
        may: { $ifNull: ["$monthlyObj.5", 0] },
        jun: { $ifNull: ["$monthlyObj.6", 0] },
        jul: { $ifNull: ["$monthlyObj.7", 0] },
        aug: { $ifNull: ["$monthlyObj.8", 0] },
        sep: { $ifNull: ["$monthlyObj.9", 0] },
        oct: { $ifNull: ["$monthlyObj.10", 0] },
        nov: { $ifNull: ["$monthlyObj.11", 0] },
        dec: { $ifNull: ["$monthlyObj.12", 0] },
        yearlyTotal: {
          $add: [
            { $ifNull: ["$monthlyObj.1", 0] },
            { $ifNull: ["$monthlyObj.2", 0] },
            { $ifNull: ["$monthlyObj.3", 0] },
            { $ifNull: ["$monthlyObj.4", 0] },
            { $ifNull: ["$monthlyObj.5", 0] },
            { $ifNull: ["$monthlyObj.6", 0] },
            { $ifNull: ["$monthlyObj.7", 0] },
            { $ifNull: ["$monthlyObj.8", 0] },
            { $ifNull: ["$monthlyObj.9", 0] },
            { $ifNull: ["$monthlyObj.10", 0] },
            { $ifNull: ["$monthlyObj.11", 0] },
            { $ifNull: ["$monthlyObj.12", 0] },
          ],
        },
      },
    },
    ...
    // Step 9: Final projection with NaN for zero months
    {
      $project: {
        _id: 0,
        name: "$_id",
        january: { $cond: [{ $eq: ["$jan", 0] }, NaN, "$jan"] },
        february: { $cond: [{ $eq: ["$feb", 0] }, NaN, "$feb"] },
        march: { $cond: [{ $eq: ["$mar", 0] }, NaN, "$mar"] },
        april: { $cond: [{ $eq: ["$apr", 0] }, NaN, "$apr"] },
        may: { $cond: [{ $eq: ["$may", 0] }, NaN, "$may"] },
        june: { $cond: [{ $eq: ["$jun", 0] }, NaN, "$jun"] },
        july: { $cond: [{ $eq: ["$jul", 0] }, NaN, "$jul"] },
        august: { $cond: [{ $eq: ["$aug", 0] }, NaN, "$aug"] },
        september: { $cond: [{ $eq: ["$sep", 0] }, NaN, "$sep"] },
        october: { $cond: [{ $eq: ["$oct", 0] }, NaN, "$oct"] },
        november: { $cond: [{ $eq: ["$nov", 0] }, NaN, "$nov"] },
        december: { $cond: [{ $eq: ["$dec", 0] }, NaN, "$dec"] },
      },
    },
  ]).toArray();
...
export async function getUnitsOtSummary(startDate, endDate, dept) {
  const result = await OVERTIME_COLLECTION.aggregate([
    {
      $match: {
        Date: { $gte: startDate, $lte: endDate },
    ...
    },
    {
      $addFields: {
        totalOtHours: {
          $divide: [{ $ceil: { $multiply: ["$totalOtHours", 10] } }, 10],
        },
      },
    },
    { $sort: { _id: 1 } },
  ...
  return result.map((entry) => ({
    Unit: entry._id,
    totalOtHours: entry.totalOtHours,
  }));
}
